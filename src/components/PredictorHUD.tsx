...
import { Settings, Key, ShieldCheck, AlertCircle, Copy, ExternalLink, Bot, CheckCircle2 } from 'lucide-react';
...
export const PredictorHUD: React.FC<PredictorHUDProps> = ({
  ...
  const isBig = (prediction?.bs ?? 'SMALL') === 'BIG';
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* 4. TOP TITLE PILL: "ORANGE APEX UI" */}
        <div className="flex items-center justify-center mb-2 relative">
          <div
            className="px-5 py-1.5 rounded-full border border-amber-500/60 flex items-center justify-center gap-1.5 shadow-md relative"
            style={{
              background: 'linear-gradient(180deg, #78350f 0%, #451a03 100%)',
              boxShadow: 'inset 0 1px 2px rgba(254, 215, 170, 0.3), 0 2px 8px rgba(0,0,0,0.6)'
            }}
          >
            <span className="w-1.5 h-1.5 rounded-full bg-orange-400 shadow-[0_0_6px_#ea580c] animate-pulse" />
            <span
              className="font-mono font-black text-xs tracking-wider text-amber-200 uppercase drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)]"
        ...
        <div className="flex items-center justify-between gap-1 mb-2 px-1">
          ...
          >
            SYS: <span className="text-amber-100 font-black">{isUnlocked ? 'LOCKED' : 'VIP AUTH'}</span>
          </div>
          ...
          >
            CORE: <span className="text-amber-100 font-black">{isUnlocked ? 'SYNC' : 'GATEWAY'}</span>
          </div>
          ...
          >
            PING: <span className="text-amber-100 font-black">19MS</span>
          </div>
        </div>
          ...
          <>
            ...
            >
              ...
              <div className="flex items-center justify-end gap-1 mb-1 pr-1 font-mono text-[8px] font-bold text-amber-400">
                <span className={`w-1.5 h-1.5 rounded-full inline-block ${isLoading ? 'bg-yellow-300 animate-ping' : 'bg-amber-400 animate-pulse shadow-[0_0_6px_#f59e0b]'}`} />
                <span className="tracking-widest">{isLoading ? 'CALCULATING 3D...' : 'SIGNAL LOCKED'}</span>
              </div>
              ...
              <div className="flex items-center justify-between px-1 mt-1 pt-1 border-t border-amber-900/60 font-mono text-[9px] font-black text-amber-300">
                ...
                <span className="text-[8px] text-amber-400/80 font-bold">DRAW: {countdown}s</span>
              </div>
            </div>
        ...
        <div className="flex items-center justify-between gap-3 font-mono">
          {/* ▶ NEXT BUTTON (Only active when unlocked) */}
          {isUnlocked ? (
            <button
              id="execute-btn"
              onClick={() => {
              ...
              disabled={isLoading}
              className="flex-1 py-2 px-3 rounded-full font-black text-[11px] tracking-wider uppercase transition-all duration-200 flex items-center justify-center gap-1.5 cursor-pointer shadow-lg active:scale-95 disabled:opacity-60"
              style={{
                background: 'linear-gradient(180deg, #f97316 0%, #ea580c 45%, #9a3412 100%)',
                border: '1.5px solid #fef08a',
                color: '#ffffff',
            ...
            >
              <span>{isLoading ? '⚡' : '▶'}</span>
              <span>{isLoading ? '3D LOADING' : 'NEXT'}</span>
            </button>
          ) : (
        ...
        <div className="flex items-center justify-between px-2 mt-2 pt-1 border-t border-amber-900/40 font-mono text-[7px] text-amber-400/80">
          <span>{isUnlocked ? `DRAW IN: ${countdown}s` : 'GAME ACTIVE & FREE'}</span>
